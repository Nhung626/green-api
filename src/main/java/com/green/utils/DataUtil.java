...
import static com.green.utils.DateTimeUtils.*;
...
public class DataUtil {
    ...
    public static boolean validatePassword(String password) {
        ...
        validate.getDetails().stream().findFirst().ifPresent(
                ruleResultDetail -> log.warn("Password is not valid: {}", ruleResultDetail.getErrorCode()));
    ...
    public static String generatePassword(){
        ...
        String allCharacters = UPPER_CASE_LETTER + LOWER_CASE_LETTER + DIGITS + SPECIAL_CHARACTERS;

        Random random = new Random();
        StringBuilder password = new StringBuilder();

        while (true) {
            for (int i = 0; i < 10; i++) { // 10 is the desired password length
            ...
            password.setLength(0); // Clear the current password if it's not valid
        }
    }
}