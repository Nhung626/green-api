...
//        String sqlGetData = ;
        ...
        StringBuilder sqlConditional = new StringBuilder("select cp.id, cp.parent_id, cp.user_id, uf.name as user_name, cp.post_id, cp.content, count(cp_child.id) as countReply, " +
        ...
        sqlConditional.append(" from comment_post cp ")
                ...
                .append(" inner join user_info uf on cp.user_id = uf.user_id and uf.status <> 2 ");
